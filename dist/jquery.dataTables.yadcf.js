...
class Yadcf {
    ...
    static options = {};
    static plugins = {};
    static exFilterColumnQueue = [];
    static selectElementCustomInitFunc;
    static selectElementCustomRefreshFunc;
    static selectElementCustomDestroyFunc;
    static yadcfDelay = (function () {
        let timer = 0;
     ...
     * @since 0.0.1
     */
    static dTXhrComplete;
    static ctrlPressed = false;
    static closeBootstrapDatepicker = false;
    static closeBootstrapDatepickerRange = false;
    static closeSelect2 = false;
    /**
     * Primary constructor.
     *
     * @returns yadcf
    ...
    static init(oTable, options_arg, params) {
        let instance = oTable.fnSettings()[0].oInstance, i = 0, tableSelector = '#' + oTable.table().node().id, selector, tmpParams;
        this.extendJQuery();
    ...
    static initDefaults(params) {
        return $.extend(true, this.default_options, params);
    }
    static getOptions(selector) {
        return this.options[selector];
    }
    static eventTargetFixUp(pEvent) {
        if (pEvent.target === undefined) {
            $.extend(pEvent, { target: pEvent.srcElement });
        }
        return pEvent;
    }
    static generateTableSelectorJQFriendly2(obj) {
        let tmpStr;
    ...
    static generateTableSelectorJQFriendlyNew(tmpStr) {
        tmpStr = this.replaceAll(tmpStr, ':', '-');
        tmpStr = this.replaceAll(tmpStr, '(', '');
        tmpStr = this.replaceAll(tmpStr, ')', '');
        tmpStr = this.replaceAll(tmpStr, ',', '');
    ...
    static initOnDtXhrComplete(initFunc) {
        this.dTXhrComplete = initFunc;
    }
    static doFilterCustomDateFunc(arg, table_selector_jq_friendly, column_number) {
        let oTable = this.oTables[table_selector_jq_friendly], columnObj = this.getOptions(oTable.selector)[column_number], yadcfState;
    ...
    static doFilter(arg, table_selector_jq_friendly, column_number, filter_match_mode) {
        $.fn.dataTable.ext.iApiIndex = this.oTablesIndex[table_selector_jq_friendly];
        let oTable = this.oTables[table_selector_jq_friendly], exclude_checked = false, settingsDt = this.getSettingsObjFromTable(oTable), selected_value, selector, data_value, column_number_filter, columnObj;
        ...
        if (arg === 'exclude' && $(`#yadcf-filter-${table_selector_jq_friendly}-${column_number}`).find('option:selected').val().trim() === '-1') {
            return;
        }
        $(`#yadcf-filter-${table_selector_jq_friendly}-${column_number}`).addClass('inuse');
        selector = arg === 'exclude' ? `#yadcf-filter-${table_selector_jq_friendly}-${column_number}` : arg;
        selected_value = $(selector).find('option:selected').val().trim();
    ...
    static doFilterMultiSelect(arg, table_selector_jq_friendly, column_number, filter_match_mode) {
        $.fn.dataTable.ext.iApiIndex = this.oTablesIndex[table_selector_jq_friendly];
        let oTable = this.oTables[table_selector_jq_friendly], selected_values = $(arg).val(), selected_values_trimmed = [], i, stringForSearch, column_number_filter, settingsDt = this.getSettingsObjFromTable(oTable);
        column_number_filter = this.calcColumnNumberFilter(settingsDt, column_number, table_selector_jq_friendly);
    ...
    static doFilterAutocomplete(arg, table_selector_jq_friendly, column_number, filter_match_mode) {
        $.fn.dataTable.ext.iApiIndex = this.oTablesIndex[table_selector_jq_friendly];
        let oTable = this.oTables[table_selector_jq_friendly], settingsDt = this.getSettingsObjFromTable(oTable), column_number_filter;
        column_number_filter = this.calcColumnNumberFilter(settingsDt, column_number, table_selector_jq_friendly);
        if (arg === 'clear') {
            if (this.exGetColumnFilterVal(oTable, column_number) === '') {
                return;
    ...
    static dateSelectSingle(pDate, pEvent, clear) {
        let oTable, date, event, column_number, dashIndex, table_selector_jq_friendly, column_number_filter, settingsDt, columnObj, yadcfState;
        if (pDate) {
            if (pDate.type === 'dp') {
                event = pDate.target;
            }
    ...
    static rangeClear(table_selector_jq_friendly, event, column_number) {
        const oTable = this.oTables[table_selector_jq_friendly];
        let yadcfState, settingsDt, column_number_filter, currentFilterValues, columnObj, fromId = `yadcf-filter-${table_selector_jq_friendly}-from-date-${column_number}`, toId = ``, $fromInput, $toInput;
        $.fn.dataTable.ext.iApiIndex = this.oTablesIndex[table_selector_jq_friendly];
    ...
    static rangeNumberSliderClear(table_selector_jq_friendly, event) {
        const oTable = this.oTables[table_selector_jq_friendly];
        let min_val, max_val, currentFilterValues, column_number;
        event = this.eventTargetFixUp(event);
        $.fn.dataTable.ext.iApiIndex = this.oTablesIndex[table_selector_jq_friendly];
    ...
    static dateKeyUP(table_selector_jq_friendly, date_format, event) {
        let oTable, date, dateId, column_number, columnObj;
        event = this.eventTargetFixUp(event);
    ...
    static rangeDateKeyUP(table_selector_jq_friendly, date_format, event) {
        let oTable, min, max, fromId, toId, column_number, columnObj, keyUp, settingsDt, column_number_filter, dpg, minTmp, maxTmp;
        event = this.eventTargetFixUp(event);
        $.fn.dataTable.ext.iApiIndex = this.oTablesIndex[table_selector_jq_friendly];
        oTable = this.oTables[table_selector_jq_friendly];
    ...
    static rangeNumberKeyUP(table_selector_jq_friendly, event) {
        const oTable = this.oTables[table_selector_jq_friendly];
        let min, max, fromId, toId, yadcfState, column_number, columnObj, keyUp, settingsDt, column_number_filter, exclude_checked = false, null_checked = false, checkbox;
        event = this.eventTargetFixUp(event);
    ...
    static doFilterMultiTablesMultiSelect(tablesSelectors, event, column_number_str, clear) {
        const columnsObj = this.getOptions(`${tablesSelectors}_${column_number_str}`)[column_number_str];
        let regex = false, smart = true, caseInsen = true, tablesAsOne, tablesArray = this.oTables[tablesSelectors], selected_values = $(event.target).val(), i;
        event = this.eventTargetFixUp(event);
    ...
    static doFilterMultiTables(tablesSelectors, event, column_number_str, clear) {
        const columnsObj = this.getOptions(tablesSelectors + '_' + column_number_str)[column_number_str];
        let regex = false, smart = true, caseInsen = true, tablesArray = this.oTables[tablesSelectors], searchVal, tablesAsOne;
        event = this.eventTargetFixUp(event);
        tablesAsOne = new $.fn.dataTable.Api(tablesArray);
    ...
    static textKeyUpMultiTables(tablesSelectors, event, column_number_str, clear) {
        let caseInsen = true, smart = true, regex = false, columnsObj = this.getOptions(`${tablesSelectors}_${column_number_str}`)[column_number_str], tablesArray = this.oTables[tablesSelectors], keyUp, serachVal, tablesAsOne;
        event = this.eventTargetFixUp(event);
        tablesAsOne = new $.fn.dataTable.Api(tablesArray);
    ...
    static textKeyUP(ev, table_selector_jq_friendly, column_number, clear) {
        const oTable = this.oTables[table_selector_jq_friendly], settingsDt = this.getSettingsObjFromTable(oTable);
        let null_checked = false, keyCodes = [37, 38, 39, 40, 17], keyUp, columnObj, exclude, column_number_filter, regex_check_box;
        ...
        keyUp = function (table_selector_jq_friendly, column_number, clear) {
            let fixedPrefix = '';
            if (settingsDt.fixedHeader !== undefined && $('.fixedHeader-floating').is(':visible')) {
    ...
    static nullChecked(ev, table_selector_jq_friendly, column_number) {
        const oTable = this.oTables[table_selector_jq_friendly];
        let settingsDt = this.getSettingsObjFromTable(oTable), exclude_checked = false, column_number_filter, click, columnObj, yadcfState, null_checked;
        column_number_filter = this.calcColumnNumberFilter(settingsDt, column_number, table_selector_jq_friendly);
        columnObj = this.getOptions(oTable.selector)[column_number];
        let fixedPrefix = '';
        if (settingsDt.fixedHeader !== undefined && $('.fixedHeader-floating').is(':visible')) {
    ...
    static autocompleteKeyUP(table_selector_jq_friendly, event) {
        const keyCodes = [37, 38, 39, 40];
        let oTable, column_number;
        event = this.eventTargetFixUp(event);
    ...
    static initMultipleTables(tablesArray, filtersOptions) {
        let i, tablesSelectors = '', default_options = {
            filter_type: 'text',
            filter_container_id: '',
    ...
    static initMultipleColumns(table, filtersOptions) {
        let tablesArray = [];
    ...
    static preventDefaultForEnter(evt) {
    ...
    static exFilterColumn(table_arg, col_filter_arr, ajaxSource = true) {
        let table_selector_jq_friendly, j, tmpStr, column_number, column_position, filter_value, fromId, toId, sliderId, optionsObj, min, max, exclude = false;
    ...
    static exGetColumnFilterVal(table_arg, column_number) {
        let retVal, fromId, toId, table_selector_jq_friendly, optionsObj, $filterElement;
        // Check if the table arg is from new datatables API (capital 'D').
        if (table_arg.settings !== undefined) {
            table_arg = table_arg.settings()[0].oInstance;
        }
    ...
    static exResetAllFilters(table_arg, noRedraw, columns) {
        let table_selector_jq_friendly, column_number, fromId, toId, sliderId, tableOptions, optionsObj, columnObjKey, settingsDt, i, $filterElement;
        // Check if the table arg is from new datatables API (capital 'D').
        if (table_arg.settings !== undefined) {
            table_arg = table_arg.settings()[0].oInstance;
        }
    ...
    static exResetFilters(table_arg, columns, noRedraw) {
        this.exResetAllFilters(table_arg, noRedraw, columns);
    }
    static exFilterExternallyTriggered(table_arg) {
        let columnsObj, columnObjKey, columnObj, filterValue, filtersValuesSingleElem, filtersValuesArr = [];
        // Check if the table arg is from new datatables API (capital 'D').
        if (table_arg.settings !== undefined) {
            table_arg = table_arg.settings()[0].oInstance;
        }
    ...
    static exRefreshColumnFilterWithDataProp(table_arg, col_num, updatedData) {
        if (table_arg.settings !== undefined) {
            table_arg = table_arg.settings()[0].oInstance;
        }
    ...
    static getSettingsObjFromTable(dt) {
        let oConfig;
        if ($.fn.dataTable.Api) {
    ...
    static arraySwapValueWithIndex(pArray) {
        let tmp = [], i;
    ...
    static arraySwapValueWithIndex2(pArray) {
        let tmp = [], i;
        for (i = 0; i < pArray.length; i++) {
    ...
    static initColReorder2(settingsDt, table_selector_jq_friendly) {
    ...
    static initColReorderFromEvent(table_selector_jq_friendly) {
        this.plugins[table_selector_jq_friendly] = undefined;
    }
    static columnsArrayToString(column_number) {
        const column_number_obj = {
            column_number_str: '',
            column_number: []
        };
    ...
    static dot2obj(tmpObj, dot_refs) {
        let i = 0;
    ...
    static setOptions(selector_arg, options_arg, params, table) {
        let tmpOptions = {}, i, col_num_as_int;
    ...
    static check3rdPPluginsNeededClose() {
    ...
    static yadcfVersionCheck(version) {
        let aThis = $.fn.dataTable.ext.sVersion.split('.'), aThat = version.split('.'), iThis, iThat, i, iLen;
    ...
    static resetIApiIndex() {
        $.fn.dataTable.ext.iApiIndex = 0;
    }
    static escapeRegExp(string) {
        return string.replace(/([.*+?^=!:${}()|'\[\]\/\\])/g, '\\$1');
    }
    static escapeRegExpInArray(arr) {
        let i;
    ...
    static getTableId(obj) {
        let tableId;
        if (obj.table !== undefined) {
            const node = obj.table().node();
    ...
    static initializeSelectPlugin(select_type, selectObject, select_type_options) {
    ...
    static refreshSelectPlugin(columnObj, selectObject, val) {
        const $selectObject = typeof selectObject === 'string' ? $(selectObject) : selectObject;
        let select_type = columnObj.select_type, select_type_options = columnObj.select_type_options;
    ...
    static initSelectPluginCustomTriggers(initFunc, refreshFunc, destroyFunc) {
    ...
    static yadcfMatchFilterString(table_arg, column_number, selected_value, filter_match_mode, multiple, exclude) {
        let case_insensitive = Yadcf.getOptions(table_arg.selector)[column_number].case_insensitive, ret_val;
        if (!selected_value) {
            return '';
        }
    ...
    static yadcfMatchFilter(oTable, selected_value, filter_match_mode, column_number, exclude, original_column_number) {
        let columnObj = this.getOptions(oTable.selector)[original_column_number], case_insensitive = columnObj.case_insensitive;
        if (exclude !== true) {
            if (filter_match_mode === 'contains') {
    ...
    static yadcfParseMatchFilter(tmpStr, filter_match_mode) {
        let retVal;
        if (filter_match_mode === 'contains') {
            retVal = tmpStr;
        }
    ...
    static calcColumnNumberFilter(settingsDt, column_number, table_selector_jq_friendly) {
    ...
    static stateSaveNullSelect(oTable, columnObj, table_selector_jq_friendly, column_number, filter_match_mode, exclude_checked) {
    ...
    static yadcfParseMatchFilterMultiSelect(tmpStr, filter_match_mode) {
        let retVal;
        if (filter_match_mode === 'contains') {
            retVal = tmpStr;
        }
    ...
    static autocompleteSelect = (event, ui) => {
        let table_column, dashIndex, table_selector_jq_friendly, col_num, filter_match_mode;
        event = this.eventTargetFixUp(event);
    ...
    static sortNumAsc(a, b) {
        return a - b;
    }
    static sortNumDesc(a, b) {
        return b - a;
    }
    static findMinInArray(array, columnObj) {
        let narray = [], i, num, min;
    ...
    static findMaxInArray(array, columnObj) {
        let narray = [], i, num, max;
        for (i = 0; i < array.length; i++) {
            if (array[i] !== null) {
                if (columnObj.ignore_char !== undefined) {
                    array[i] = array[i].toString().replace(columnObj.ignore_char, '');
    ...
    static addRangeNumberAndSliderFilterCapability(table_selector_jq_friendly, fromId, toId, col_num, ignore_char, sliderMaxMin) {
        $.fn.dataTable.ext.afnFiltering.push(function (settingsDt, aData, iDataIndex, rowData) {
            let min, max, val, retVal = false, table_selector_jq_friendly_local = table_selector_jq_friendly, current_table_selector_jq_friendly = Yadcf.generateTableSelectorJQFriendly2(settingsDt), ignore_char_local = ignore_char, column_data_type, html_data_type, columnObj, column_number_filter, valFrom, valTo, exclude_checked = false;
    ...
    static addCustomFunctionFilterCapability(table_selector_jq_friendly, filterId, col_num) {
        $.fn.dataTable.ext.afnFiltering.push(function (settingsDt, aData, iDataIndex, stateVal) {
            let filterVal = $('#' + filterId).val(), columnVal, retVal = false, table_selector_jq_friendly_local = table_selector_jq_friendly, current_table_selector_jq_friendly = Yadcf.generateTableSelectorJQFriendly2(settingsDt), custom_func, column_number_filter;
    ...
    static addRangeDateFilterCapability(table_selector_jq_friendly, fromId, toId, col_num, date_format) {
        $.fn.dataTable.ext.afnFiltering.push(function (settingsDt, aData, iDataIndex, rowData) {
            let min = document.getElementById(fromId) !== null ? document.getElementById(fromId).value : '', max = document.getElementById(toId) !== null ? document.getElementById(toId).value : '', val, retVal = false, table_selector_jq_friendly_local = table_selector_jq_friendly, current_table_selector_jq_friendly = Yadcf.generateTableSelectorJQFriendly2(settingsDt), column_data_type, html_data_type, columnObj, column_number_filter, min_time, max_time, dataRenderFunc, dpg;
    ...
    static addNullFilterCapability(table_selector_jq_friendly, col_num, isSelect) {
        $.fn.dataTable.ext.afnFiltering.push(function (settingsDt, aData, iDataIndex, rowData) {
            let val, retVal = false, table_selector_jq_friendly_local = table_selector_jq_friendly, current_table_selector_jq_friendly = Yadcf.generateTableSelectorJQFriendly2(settingsDt), columnObj, column_number_filter, null_checked, exclude_checked;
            if (table_selector_jq_friendly_local !== current_table_selector_jq_friendly) {
                return true;
            }
            columnObj = Yadcf.getOptions(settingsDt.oInstance.selector)[col_num];
    ...
    static addRangeNumberFilter(filter_selector_string, table_selector_jq_friendly, column_number, filter_reset_button_text, filter_default_label, ignore_char) {
        let fromId = `yadcf-filter-${table_selector_jq_friendly}-from-${column_number}`, toId = `yadcf-filter-${table_selector_jq_friendly}-to-${column_number}`, filter_selector_string_tmp, filter_wrapper_id, oTable, columnObj, filterActionFn, null_str, exclude_str;
        ...
        $(filter_selector_string).append(this.makeElement('<input>', {
    ...
    static dateSelect(pDate, pEvent) {
        let oTable, column_number, dashIndex, table_selector_jq_friendly, yadcfState, from, to, event, columnObj, column_number_filter, settingsDt, keyUp;
    ...
    static addRangeDateFilter(filter_selector_string, table_selector_jq_friendly, column_number, filter_reset_button_text, filter_default_label, date_format) {
        let fromId = `yadcf-filter-${table_selector_jq_friendly}-from-date-${column_number}`, toId = `yadcf-filter-${table_selector_jq_friendly}-to-date-${column_number}`, filter_selector_string_tmp, filter_wrapper_id, oTable, columnObj, datepickerObj = {}, filterActionFn, $fromInput, $toInput, innerWrapperAdditionalClass = '';
        filter_wrapper_id = `yadcf-filter-wrapper-${table_selector_jq_friendly}-${column_number}`;
        if ($('#' + filter_wrapper_id).length > 0) {
            return;
        }
    ...
    static addDateFilter(filter_selector_string, table_selector_jq_friendly, column_number, filter_reset_button_text, filter_default_label, date_format) {
        let dateId = `yadcf-filter-${table_selector_jq_friendly}-${column_number}`, filter_selector_string_tmp, filter_wrapper_id, oTable, columnObj, datepickerObj = {}, filterActionFn, settingsDt;
        filter_wrapper_id = `yadcf-filter-wrapper-${table_selector_jq_friendly}-${column_number}`;
        if ($('#' + filter_wrapper_id).length > 0) {
            return;
        }
    ...
    static rangeNumberSldierDrawTips(min_tip_val, max_tip_val, min_tip_id, max_tip_id, table_selector_jq_friendly, column_number) {
    ...
    static rangeNumberSliderChange(table_selector_jq_friendly, event, ui) {
        let oTable, min_val, max_val, slider_inuse, yadcfState, column_number, columnObj, keyUp, settingsDt, column_number_filter;
        event = this.eventTargetFixUp(event);
    ...
    static addRangeNumberSliderFilter(filter_selector_string, table_selector_jq_friendly, column_number, filter_reset_button_text, min_val, max_val, ignore_char) {
        let sliderId = `yadcf-filter-${table_selector_jq_friendly}-slider-${column_number}`, min_tip_id = `yadcf-filter-${table_selector_jq_friendly}-min_tip-${column_number}`, max_tip_id = `yadcf-filter-${table_selector_jq_friendly}-max_tip-${column_number}`, filter_selector_string_tmp, filter_wrapper_id, oTable, min_state_val = min_val, max_state_val = max_val, columnObj, slideFunc, changeFunc, sliderObj, sliderMaxMin = {
            min: min_val,
            max: max_val
        }, settingsDt, currSliderMin = $('#' + sliderId).slider('option', 'min'), currSliderMax = $('#' + sliderId).slider('option', 'max'), redrawTable;
        filter_wrapper_id = `yadcf-filter-wrapper-${table_selector_jq_friendly}-${column_number}`;
    ...
    static destroyThirdPartyPlugins(table_arg) {
        let tableOptions, table_selector_jq_friendly, columnObjKey, column_number, optionsObj, fromId, toId;
        // Check if the table arg is from new datatables API (capital 'D').
        if (table_arg.settings !== undefined) {
            table_arg = table_arg.settings()[0].oInstance;
        }
    ...
    static sortColumnData(column_data, columnObj) {
    ...
    static parseTableColumn(pTable, columnObj, table_selector_jq_friendly, pSettings) {
        let col_filter_array = {}, column_data = [], col_inner_elements, col_inner_data, col_inner_data_helper, j, k, data, data_length, settingsDt, column_number_filter;
                    ...
                    for (k = 0; k < col_inner_elements.length; k++) {
                        ...
                        if (col_inner_data || col_inner_data_helper) {
                            if (!col_inner_data_helper) {
                                ...
                                col_inner_data_helper.each((index) => {
    ...
    static makeElement(element, opts) {
    ...
    static appendFilters(oTable, args, table_selector, pSettings) {
        ...
        for (columnObjKey in args) {
            if (args.hasOwnProperty(columnObjKey)) {
                ...
                if ($filter_selector.length === 1) {
                    ...
                    if (columnObj.filter_type === 'select' || columnObj.filter_type === 'custom_func') {
                        ...
                        if (columnObj.filter_type === 'select') {
                            ...
                            $(filter_selector_string).append(this.makeElement('<select>', {
                            ...
                            if (filter_reset_button_text !== false) {
                                $(filter_selector_string).find('.yadcf-filter').after(this.makeElement('<button>', {
                                    type: 'button',
                                    id: `yadcf-filter-${table_selector_jq_friendly}-${column_number}` + '-reset',
                                    onmousedown: this.stopPropagation,
                                    onclick: function (colNo, tableSel) {
                            ...
                            if (columnObj.externally_triggered_checkboxes_text &&
                                ...
                                $(filter_selector_string).append(this.makeElement('<button>', {
                                    type: 'button',
                                    id: `yadcf-filter-${table_selector_jq_friendly}-${column_number}` + '-externally_triggered_checkboxes-button',
                                    onmousedown: this.stopPropagation,
                                    onclick: this.stopPropagation,
                ...
                if ($(document).data(`#yadcf-filter-${table_selector_jq_friendly}-${column_number}_val`) !== undefined &&
                    $(document).data(`#yadcf-filter-${table_selector_jq_friendly}-${column_number}_val`) !== '-1') {
    ...
    static loadFromStateSaveTextFilter(oTable, settingsDt, columnObj, table_selector_jq_friendly, column_position, column_number) {
        if (columnObj.null_check_box === true) {
            if (settingsDt.oFeatures.bStateSave === true && settingsDt.oLoadedState) {
                if (settingsDt.oLoadedState.yadcfState &&
                    settingsDt.oLoadedState.yadcfState[table_selector_jq_friendly] &&
    ...
    static saveTextKeyUpState(oTable, table_selector_jq_friendly, column_number, regex_check_box, null_checked, exclude) {
        let yadcfState;
        if (oTable.fnSettings().oFeatures.bStateSave === true &&
            oTable.fnSettings().oLoadedState &&
    ...
    static scrollXYHandler(oTable, table_selector) {
        let $tmpSelector, filters_position = $(document).data(`${table_selector}_filters_position`), table_selector_jq_friendly = this.generateTableSelectorJQFriendly2(oTable);
        if (filters_position === 'thead') {
    ...
    static initAndBindTable(oTable, table_selector, index, pTableDT) {
        const table_selector_jq_friendly = this.generateTableSelectorJQFriendly2(oTable);
        let table_selector_tmp;
        ...
        if (this.isDOMSource(oTable)) {
            ...
            if (this.yadcfVersionCheck('1.10')) {
                $(document).off('xhr.dt', oTable.selector).on('xhr.dt', oTable.selector, (e, settings, json) => {
                    let col_num, column_number_filter, table_selector_jq_friendly = this.generateTableSelectorJQFriendly2(oTable);
                    if (!json) {
        ...
        if (this.yadcfVersionCheck('1.10')) {
            $(document).off('stateLoaded.dt', oTable.selector).on('stateLoaded.dt', oTable.selector, (event, settings) => {
            ...
            $(document).off('column-visibility.dt', oTable.selector).on('column-visibility.dt', oTable.selector, (e, settings, col_num, state) => {
                const obj = {}, columnsObj = this.getOptions(settings.oInstance.selector);
                if (state === true && settings._oFixedColumns === undefined) {
                    ...
                    this.appendFilters(this.oTables[this.generateTableSelectorJQFriendly2(settings)], columnsObj, settings.oInstance.selector, settings);
                }
            });
            ...
            $(document).off('destroy.dt', oTable.selector).on('destroy.dt', oTable.selector, (e, ui) => {
                this.removeFilters(oTable);
                this.removeFilters(ui.oInstance);
            });
        }
        else {
            $(document).off('draw', oTable.selector).on('draw', oTable.selector, (e, settings) => {
                this.appendFilters(oTable, this.getOptions(settings.oInstance.selector), settings.oInstance.selector, settings);
            });
            $(document).off('destroy', oTable.selector).on('destroy', oTable.selector, (e, ui) => {
                this.removeFilters(oTable);
    ...
    static extendJQuery() {
        $.extend($.fn, {
            yadcf: function (options_arg, params) {
                const tableSelector = '#' + this.fnSettings().sTableId;
                let i = 0, tmpParams, selector;
                // In case that instance.selector will be undefined (jQuery 3).
    ...
    static close3rdPPluginsNeededClose(evt) {
    ...
    static exInternalFilterColumnAJAXQueue(table_arg, col_filter_arr) {
        return function () {
            Yadcf.exFilterColumn(table_arg, col_filter_arr, true);
        };
    }
    static clearStateSave(oTable, column_number, table_selector_jq_friendly) {
        let yadcfState;
        if (oTable.fnSettings().oFeatures.bStateSave === true) {
            if (!oTable.fnSettings().oLoadedState) {
                oTable.fnSettings().oLoadedState = {};
    ...
    static saveStateSave(oTable, column_number, table_selector_jq_friendly, from, to) {
        let yadcfState;
        if (oTable.fnSettings().oFeatures.bStateSave === true) {
            if (!oTable.fnSettings().oLoadedState) {
                oTable.fnSettings().oLoadedState = {};
    ...
    static resetExcludeRegexCheckboxes(selector) {
